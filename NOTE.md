...
from the repo. The measurement-honesty material that used to live here is
preserved in `NOTE_previous.md`.*
...
enrichment → applicant scoring → introduction engine → operator UI. **330 tests.**
...
data/raw/people_raw.json      299 messy records, seeded noise
  ...
  survivorship                39 merged clusters, 3 held on a field conflict
        ↓
  AI enrichment               258/258, closed enums, every quote verified
        ↓
  applicant scoring           deterministic rubric; LLM writes prose from it
        ↓
  introduction engine         local embeddings, 267 suggestions, 266 drafted
        ↓
...
**1,087 of 1,095 quotes verified — 99.3%.** 8 records of 258 carry at least one
...
the abstraction was real, switching to Gemini took one file. Cache keys already
include the provider name, so Groq's cached answers stayed valid.
...
¹ 258 people enriched, 40 scores explained, 266 intro drafts.
...
258 canonical people, 227 actionable, 25,651 pairs considered, 8,496 above the
floor, **267 suggestions** (top 3 per person, capped on both sides), 246
reciprocal, 185 people with a suggestion. Embedding: 18.7s, local CPU.

**Rejected by filter — deterministic, before any model call:**

| filter | count |
|---|---|
| no_shared_signal | 17,121 |
| competitors | 32 |
...
**The 92% reciprocity rate is a synthetic-data artifact.** 246 of 267 suggestions
...
**The competitor filter is barely exercised.** 32 firings out of 25,651 pairs —
and only after enrichment reached 100%; before that it fired 6 times. It is
deliberately narrow (both founders, same sector, adjacent stages) and on this data
it has not been meaningfully tested.
